...
public class ComponentBag extends Item {
    public ComponentBag(Properties pProperties) {
            ...
            NetworkHooks.openScreen(serverPlayer,
                    new SimpleMenuProvider(
                            (windowId, playerInventory, player) ->
                                    new ComponentBagMenu(windowId, playerInventory, stack),
                            stack.getHoverName()
                    )
            );
        }
        ...
        return new ICapabilityProvider() {
            ...
            public <T> LazyOptional<T> getCapability(Capability<T> cap, @Nullable Direction side) {
                if (cap == ForgeCapabilities.ITEM_HANDLER) {
                    return LazyOptional.of(() -> new ComponentBagItemStackHandler(stack, 9)).cast();
                }
