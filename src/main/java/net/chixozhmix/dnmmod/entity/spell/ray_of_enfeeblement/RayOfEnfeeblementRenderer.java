...
public class RayOfEnfeeblementRenderer extends EntityRenderer<RayOfEnfeeblement> {
    public static final ModelLayerLocation MODEL_LAYER_LOCATION = new ModelLayerLocation(
            new ResourceLocation(DnMmod.MOD_ID, "ray_of_enfeeblement_model"), "main");
    ...
    @Override
    public void render(RayOfEnfeeblement pEntity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        float lifetime = 15.0F;
        float scalar = 0.25F;
        float length = 32.0F * scalar * scalar;
        float f = (float)pEntity.tickCount + pPartialTick;
        pPoseStack.translate((double)0.0F, pEntity.getBoundingBox().getYsize() * (double)0.5F, (double)0.0F);
        ...
        for(float i = 0.0F; i < pEntity.distance * 4.0F; i += length) {
            ...
            VertexConsumer consumer = pBuffer.getBuffer(RenderType.energySwirl(TEXTURE_OVERLAY, 0.0F, 0.0F));
            pPoseStack.pushPose();
            ...
            this.body.render(pPoseStack, consumer, 15728880, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, alpha);
            pPoseStack.popPose();
            consumer = pBuffer.getBuffer(RenderType.energySwirl(TEXTURE_CORE, 0.0F, 0.0F));
            pPoseStack.pushPose();
            expansion = Mth.clampedLerp(1.0F, 0.0F, f / (lifetime - 8.0F));
            pPoseStack.scale(expansion, 1.0F, expansion);
            pPoseStack.mulPose(Axis.YP.rotationDegrees(f * -10.0F));
            this.body.render(pPoseStack, consumer, 15728880, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            pPoseStack.popPose();
        }
